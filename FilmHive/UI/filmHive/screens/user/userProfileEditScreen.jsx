...
export default function UserProfileEditScreen({ navigation }) {
    ...
    const fetchUser = async () => {
        try {
            ...
            const filteredUsers = users.resultList.filter(user => user.userId != AuthProvider.userId);
    ...
    const getValidationSchema = (users) => yup.object().shape({
        ...
        username: yup.string()
            ...
            .test('unique', 'Username is already taken.', async function (value) {
                if (!value || !users) return true;
                const existingUser = await users.find(user => user.username.toLowerCase() === value.toLowerCase());
                return !existingUser;
        ...
        passwordConfirmation: yup.string().when('isChecked', {
            is: true,
            then: schema => schema
                .oneOf([yup.ref('newPassword')], 'Password confirmation must match the new password.')
