...
export default function UpdateFilmReviewScreen({ navigation }) {
  ...
  async function fetchReviews() {
    try {
      ...
      setUserReview(reviews.resultList[0]);
    } catch (error) {
      alert(error.message);
    }
  }
  ...
  async function checkFavoriteStatus() {
    try {
      ...
      if (favorites.resultList.length > 0) {
        setFavorite(favorites.resultList[0].isActive);
        return favorites.resultList[0];
      }
  ...
  async function toggleFavorite() {
    try {
      const existingFavorite = await checkFavoriteStatus();
  ...
  useEffect(() => {
    if (userReview) {
      ...
      setStarRating(userReview.grade);
    }
  }, [userReview]);
